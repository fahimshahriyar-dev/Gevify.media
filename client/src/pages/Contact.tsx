  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (draftLogo.trim() && draftLogo.trim() !== logo) {
        try {
          const logoRes = await fetch(`${API_BASE}/api/content/logo`, {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="relative w-full h-mobile-screen bg-[#06102F] overflow-hidden select-none text-white">
      ...
      >
        ...
        >
          {/* Div 1: Brand Logo + Name & Contact Info */}
          <div className="flex flex-col sm:flex-row items-center sm:items-end justify-center sm:justify-between w-full gap-4 sm:gap-0 pointer-events-auto text-center sm:text-left">
            {/* Left: Brand Logo & Name */}
            <div
              ref={brandRef}
              className="flex items-center justify-center sm:justify-start gap-2.5 sm:gap-2.5 min-w-0"
            >
              <img
                src={optimizeCloudinaryUrl(logo, 160)}
                alt={contact.company}
                className="h-9 sm:h-8 md:h-9 w-auto object-contain shrink-0"
              />
              <span className="text-xl sm:text-lg md:text-xl font-bold tracking-tight truncate">
            ...
            <div
              ref={contactRef}
              className="flex flex-col items-center sm:items-end justify-center gap-2 sm:gap-2 text-center sm:text-right min-w-0"
            >
              <div className="flex items-center justify-center gap-1.5 text-sm sm:text-xs text-zinc-400">
                <span>For queries:</span>
                <Mail className="w-4 h-4 sm:w-4 sm:h-4 text-zinc-400" />
              </div>
              <a
                href={`mailto:${contact.email}`}
                className="text-sm sm:text-sm md:text-base font-semibold text-white hover:text-[#5ACFFE] transition-colors break-all"
              >
                {contact.email}
              </a>
              <div className="flex items-center gap-6 sm:gap-4 mt-2 sm:mt-0.5">
                <a
                ...
                >
                  <svg
                    className="w-7 h-7 sm:w-5 sm:h-5 fill-current"
                    viewBox="0 0 24 24"
                  >
                ...
                >
                  <svg
                    className="w-7 h-7 sm:w-5 sm:h-5 fill-none stroke-current"
                    strokeWidth="2"
                    viewBox="0 0 24 24"
                  >
          ...
          <h1
            ref={titleRef}
            className="text-[12vw] sm:text-[10vw] md:text-[9vw] lg:text-[120px] xl:text-[140px] font-bold tracking-tighter bg-gradient-to-b from-[#333] to-[#c0c0c0] bg-clip-text text-transparent leading-none text-center sm:text-left pointer-events-auto pb-[0.15em] break-words"
          >
            {contact.title}
          </h1>
          ...
          <p
            ref={copyrightRef}
            className="text-[10px] sm:text-xs md:text-sm text-zinc-500 font-medium text-center sm:text-left pointer-events-auto"
          >
            © 2026{" "}
          ...
          <div className="w-full max-w-[700px] px-5 sm:px-10 space-y-8 sm:space-y-10">
            ...
            <div className="space-y-2 sm:space-y-3">
              ...
              <input
                ...
                className="w-full bg-transparent border-b border-zinc-700 focus:border-[#0086F0] outline-none text-xs sm:text-sm text-zinc-300 placeholder-zinc-600 pb-2.5 sm:pb-3 transition-colors duration-300"
              />
            </div>
