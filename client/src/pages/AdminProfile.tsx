...
const AdminProfile = () => {
  ...
  const handleUpdateSignupCode = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/admin/signup-code", {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`
  ...
  const handleToggleCheck = async (id: string) => {
    try {
      const res = await fetch(`http://localhost:5000/api/admin/applications/${id}/check`, {
        method: "PUT",
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleRoleChange = async (memberId: string, newRole: string) => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/admin/team/role", {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`
