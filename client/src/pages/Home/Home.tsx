...
const Home = ({ isAdminMode = false }: HomeProps) => {
  ...
  const handleUpdateHero = async (
    ...
    const token = localStorage.getItem("adminToken");
    try {
      const response = await fetch("https://api.gevify.media/api/content/hero", {
        method: "PUT",
        headers: {
  ...
  const handleUpdateFaqs = async (faqs: FaqItem[]) => {
    const token = localStorage.getItem("adminToken");
    try {
      const response = await fetch("https://api.gevify.media/api/content/faqs", {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleUpdateBrands = async (brands: BrandsData) => {
    const token = localStorage.getItem("adminToken");
    try {
      const response = await fetch("https://api.gevify.media/api/content/brands", {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
